...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function printobjectbuffer($is_table = false, $blockdir = false)
	{
		...
		foreach ($this->objectbuffer as $ib => $objattr) {
			...
			if ($objattr['type'] === 'barcode') {
				...
				$barcodeTypes = ['EAN13', 'kd_buku', 'ISSN', 'UPCA', 'UPCE', 'EAN8'];
	 ...
	 * UPC/EAN barcode
	 *
	 * EAN13, EAN8, UPCA, UPCE, kd_buku, ISSN
	 * Accepts 12 or 13 digits with or without - hyphens
	 */
	function WriteBarcode($code, $showtext = 1, $x = '', $y = '', $size = 1, $border = 0, $paddingL = 1, $paddingR = 1, $paddingT = 2, $paddingB = 2, $height = 1, $bgcol = false, $col = false, $btype = 'kd_buku', $supplement = '0', $supplement_code = '', $k = 1)
	{
		if (empty($code)) {
			return;
		}
		...
		if ((($btype === 'EAN13' || $btype === 'kd_buku' || $btype === 'ISSN') && strlen($code) === 12)
		...
		$fbwi = $fbw - 2; // Full barcode width incl. light margins - 2mm - for kd_buku string
		...
		if (($btype == 'EAN13' && $showtext) || $btype == 'ISSN' || $btype == 'kd_buku') { // Add height for kd_buku string + margin from top of bars
			$tkd_bukum = 1.5 * $size; // Top margin between kd_buku (if shown) & bars
			$codestr_fontsize = 2.1 * $size;
			$paddingT += $codestr_fontsize + $tkd_bukum;
		}
		...
		// kd_buku string
		if (($btype === 'EAN13' && $showtext) || $btype === 'kd_buku' || $btype === 'ISSN') {
			...
			$this->y = $y + $paddingT - ($codestr_fontsize ) - $tkd_bukum;
		...
		if ($btype === 'EAN13' || $btype === 'kd_buku' || $btype === 'ISSN') {
