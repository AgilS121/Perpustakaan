...
class BarCode extends Tag
{
	...
	public function open($attr, &$ahtml, &$ihtml)
	{
		...
		if (!empty($attr['CODE'])) {
			...
			if (preg_match('/^(EAN13|kd_buku|ISSN|EAN8|UPCA|UPCE)P([25])$/', $objattr['btype'], $m)) {
				$objattr['btype'] = $m[1];
				$objattr['bsupp'] = $m[2];
			...
			if (in_array($objattr['btype'], ['EAN13', 'kd_buku', 'ISSN', 'UPCA', 'UPCE', 'EAN8'])) {
				...
				// Add height for kd_buku string + margin from top of bars
				if (($objattr['showtext'] && $objattr['btype'] === 'EAN13') || $objattr['btype'] === 'kd_buku' || $objattr['btype'] === 'ISSN') {
					$tkd_bukum = 1.5 * $objattr['bsize']; // Top margin between TOP TEXT (kd_buku - if shown) & bars
					$kd_buku_fontsize = 2.1 * $objattr['bsize'];
					$h += $kd_buku_fontsize + $tkd_bukum;
				}
